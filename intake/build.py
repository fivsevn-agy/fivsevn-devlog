...
def read_yaml_blocks(markdown_text: str) -> list[tuple[str, dict[str, Any]]]:
    blocks: list[tuple[str, dict[str, Any]]] = []

    for match in FENCED_BLOCK_PATTERN.finditer(markdown_text):
        ...
        if not isinstance(parsed, dict):
            raise ValueError(f"Invalid `{kind}` block: expected YAML mapping")

        blocks.append((kind, parsed))

    return blocks


def build_config_from_sources_markdown(
    ...
    for kind, block in read_yaml_blocks(markdown_text):
        ...
        if kind == "section":
            section_id = str(block.get("id") or "").strip()
            if not section_id:
                raise ValueError("A `section` block is missing `id`.")

            sections.setdefault(
                section_id,
                {
                    "title": block.get("title", section_id.replace("_", " ")),
                    "title_zh": block.get("title_zh", ""),
                    "description": block.get("description", ""),
                    "description_zh": block.get("description_zh", ""),
                    "feeds": [],
                },
            )
            continue
        ...
        if kind == "source":
            source = dict(block)
            section_id = str(source.get("section") or "").strip()

            if not section_id:
                raise ValueError(f"A `source` block is missing `section`: {source.get('name')}")

            if not source.get("name"):
                raise ValueError(f"A `source` block in `{section_id}` is missing `name`.")

            if not source.get("site_url"):
                raise ValueError(f"Source `{source.get('name')}` is missing `site_url`.")
            ...
            if region:
                source.pop("region", None)
                sections[section_id].setdefault("regions", {})
                sections[section_id]["regions"].setdefault(region, [])
                sections[section_id]["regions"][region].append(source)
                sections[section_id].pop("feeds", None)
            else:
                sections[section_id].setdefault("feeds", [])
                sections[section_id]["feeds"].append(source)

    if categories is None:
        raise ValueError("sources.md is missing a `categories` block.")

    config = dict(base_config)

    # `sources.md` is now the canonical source registry for the
    # "On the shelf / 本日上架" resource area.
...
def write_generated_feeds_yml(config: dict[str, Any]) -> None:
    header = (
        "# GENERATED FILE. DO NOT EDIT DIRECTLY.\n"
        "# Source: intake/sources.md\n"
        "# Generated by: intake/build.py\n\n"
    )
...
def prepare_config_from_sources_if_present(base_config: dict[str, Any]) -> dict[str, Any]:
    if not SOURCE_REGISTRY_PATH.exists():
        return base_config

    markdown_text = SOURCE_REGISTRY_PATH.read_text(encoding="utf-8")
    config = build_config_from_sources_markdown(base_config, markdown_text)
    write_generated_feeds_yml(config)
...
def load_config() -> dict[str, Any]:
    ...
    return prepare_config_from_sources_if_present(base_config)
...
<body>
  <main>
    ...
    <footer>
      <p>本页面由 GitHub Actions 每日生成，数据来自 RSS 与采样脚本。</p>
    </footer>
  </main>
