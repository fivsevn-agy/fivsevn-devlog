...
OUTPUT_JSON = OUTPUT_DIR / "daily_field_sample.json"
OUTPUT_HTML = OUTPUT_DIR / "daily_field_sample.html"

# GBIF Backbone taxon key used to limit the field sample to Arthropoda.
ARTHROPODA_KEY = 54
...
def fetch_random_species(count: int) -> dict[str, Any]:
    ...
    if not candidates:
        raise RuntimeError("No usable field sample returned by GBIF.")
...
def render_html(payload: dict[str, Any]) -> str:
    ...
    return f'''<section id="daily-field-sample" class="daily-field-sample">
  <h2>{title}</h2>
...
def main() -> None:
    ...
    if count <= 0:
        raise RuntimeError("GBIF returned zero accepted Arthropoda species for the field sample.")
    ...
    for _ in range(10):
        try:
            ...
            print(f"[daily field sample] {payload['scientific_name']}")
            return
        except Exception as error:
            last_error = error

    raise RuntimeError(f"Failed to generate daily field sample: {last_error}")
