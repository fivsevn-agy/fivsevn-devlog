# fivsevn System Map for AI Collaborators

This document is the English AI-facing operational map for the fivsevn system.

Its purpose is to give any AI collaborator enough context to understand the whole structure of fivsevn: public entry points, content categories, repository responsibilities, publishing surfaces, storage rules, and routing decisions.

The Chinese `system-map.md` remains the canonical source. This English version is not a literal translation. It is an AI-optimized operational brief written to make the system easier for assistants, agents, scheduled tasks, and future automation workflows to parse and use.
...
- `fivsevn.com` — the main public website and visual entrance.
...
`fivsevn.com` is the main public-facing entrance. It should be treated as the primary external link when presenting fivsevn to other people.
...
`fivsevn.com/natsci/` is more visual and observational.
`devlog.fivsevn.com/natsci/` is more suitable for source material, articles, and structured notes.
...
At the current stage, `netcom` mainly lives in devlog because it does not require a strong visual WordPress presentation layer.
...
If the content is only for public page display, WordPress may be enough.
If it involves worldbuilding source files, settings, interactive pages, scripts, or reusable resources, use `fivsevn-57store`.
...
| Networks / communications / technical systems | `fivsevn-devlog/netcom` | `devlog/netcom` | No required WordPress display yet |
| Sources / RSS / reading sources | `fivsevn-devlog/intake` | `devlog/intake` | Not equivalent to formal notes |
...
The `ai/` folder in `fivsevn-devlog` is reserved for AI collaboration files.
...
Files in `ai/` should be stable, easy to parse, and useful for assistants. They are not primarily visual pages.

---

## 15. Minimal summary

fivsevn is a lightweight personal creative system with `fivsevn.com` as the public visual entrance, GitHub as the long-term source and archive layer, WordPress as the display layer, YouTube as the video layer, and several content zones: knowledge notes, lifestyle pages, visual expression, source intake, narrative experiment, and backend logs.
