...
用于非译文的外部链接文章，主要是 posts 中迁移自 WordPress / Home Blog 的旧文章。
...
且文章不是 `type: translation`，目录链接会优先使用 original_url，而不是仓库内 .md 文件路径。
...
注意：译文文章即使填写 original_url，目录也应链接仓库内译文页面；original_url 只作为原文来源元数据保留。
...
| 非译文且有 original_url | 是，取决于 status | 是 | 链接优先指向 original_url |
| 译文且有 original_url | 是，取决于 status | 是 | 链接仓库内译文，original_url 仅作来源元数据 |
...
5. 旧 WordPress / Home Blog 原创文章使用 original_url 保持外链。
6. 译文统一使用 type: translation，并链接仓库内译文页面。
